    ...
    document.querySelectorAll('.report-block').forEach(reportBlock => {
        ...
        if (readBtn) {
            readBtn.addEventListener('click', (e) => {
                ...
                const reportUrl = reportBlock.dataset.reportUrl;
                if (reportUrl) {
                    // Logic to open report (e.g., in a new tab or modal)
                    // window.open(reportUrl, '_blank');
                    console.log('Reading report:', reportUrl);
                    // Example: Basic alert for testing
                    alert('Reading Report: ' + reportUrl);
                }
            });
        }

        if (listenBtn && audioWrapper) {
                ...
                if (!audioWrapper.classList.contains('visible')) {
                    ...
                    if (audioElement && !audioElement.paused) {
                     ...
                     if (audioProgressBar) audioProgressBar.style.width = '0%';
                      // Reset time display (duration might not be loaded yet, handle with loadedmetadata)
                     if (audioTimeDisplay) audioTimeDisplay.textContent = '0:00 / 0:00';

                } else {
                     // If showing, attempt to load metadata if not already loaded
                      if (audioElement && isNaN(audioElement.duration)) {
                          // Trigger loading metadata if needed, although preload="auto" should handle this
                         console.log("Loading audio metadata for:", audioElement.src);
                      }
                     // Set initial button state (play icon) - playback starts on play button click
                      if(audioPlayPauseBtn) audioPlayPauseBtn.textContent = '▶';
                      if(audioPlayPauseBtn) audioPlayPauseBtn.classList.remove('playing');
                      // Set initial time display if duration is known
        ...
        // --- New Audio Player Functionality (Add these) ---
        if (audioElement && audioPlayPauseBtn && audioProgressBarContainer && audioProgressBar && audioTimeDisplay) {
            ...
            audioPlayPauseBtn.addEventListener('click', () => {
                if (audioElement.paused) {
                    // Pause all other players on the page before playing this one
                    document.querySelectorAll('.js-audio-element, .js-video-element').forEach(media => {
                         if (media !== audioElement && !media.paused) {
                             media.pause();
                              // Optionally update their play/pause buttons and classes
                             const parentPlayer = media.closest('.custom-player'); // Find the custom player container
                             if (parentPlayer) {
                                  const btn = parentPlayer.querySelector('.play-pause-btn'); // Find the play/pause button within that player
                                   if(btn) btn.textContent = '▶'; // Change button to play symbol
                                   if(btn) btn.classList.remove('playing'); // Remove playing class
                             }
                         }
                    });
                    audioElement.play();
                    audioPlayPauseBtn.textContent = '❚❚'; // Change button to pause symbol
                     audioPlayPauseBtn.classList.add('playing'); // Add playing class for styling (e.g., glow)
                } else {
                    audioElement.pause();
        ...
        // --- End New Audio Player Functionality ---
        ...
        if(audioCloseBtn && audioWrapper && audioElement) {
            audioCloseBtn.addEventListener('click', () => {
                ...
                if (!audioElement.paused) {
                 ...
                 if (audioProgressBar) audioProgressBar.style.width = '0%';
                 // Reset time display (duration might be known now, update if needed)
                  if (audioTimeDisplay && !isNaN(audioElement.duration)) {
                      audioTimeDisplay.textContent = `0:00 / ${formatTime(audioElement.duration)}`;
                  } else if (audioTimeDisplay) {
         ...
         if(videoCloseBtn && videoWrapper && videoElement) {
