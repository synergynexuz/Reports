...
window.addEventListener('load', () => {
    ...
    document.querySelectorAll('.report-block').forEach(reportBlock => {
        ...
        // --- Add logging to check source URLs ---
        console.log(`Report Block: ${reportBlock.querySelector('.report-title')?.textContent.trim()}`);
        ...
        if (listenBtn && audioWrapper && audioElement) { // Ensure audioElement exists
            listenBtn.addEventListener('click', () => {
                ...
                if (!audioWrapper.classList.contains('visible')) {
                    ...
                    if (audioSrc) { // Check if audioSrc exists
                        if (audioElement.src !== audioSrc) { // Only set src if different
                             audioElement.src = audioSrc;
                             // audioElement.load(); // load() is often not necessary with src change + preload="auto"
             ...
             watchBtn.addEventListener('click', () => {
                ...
                // Toggle visibility of the video player wrapper
                 ...
                 if (!videoWrapper.classList.contains('visible')) {
                     ...
                     if (videoSrc) { // Check if videoSrc exists
                         if (videoElement.src !== videoSrc) { // Only set src if different
                             videoElement.src = videoSrc;
                             // videoElement.load(); // load() is often not necessary with src change + preload="auto"
        ...
        if (audioElement && audioPlayPauseBtn && audioProgressBarContainer && audioProgressBar && audioTimeDisplay) {
            ...
            // Update total duration when metadata is loaded
             ...
             audioElement.addEventListener('error', (e) => {
                 ...
                 if (audioPlayPauseBtn) audioPlayPauseBtn.textContent = "Error";
                 if (audioPlayPauseBtn) audioPlayPauseBtn.classList.remove('playing');
             });
