...
(function() { // Use an IIFE to keep variables out of the global scope
    ...
    function initializeReportBlock(containerElement) {
      ...
      // First, ensure the listen button is hidden by default
      if (listenBtn) {
      ...
      if (audioSrc) {
        ...
        if (audio && audioWrapper && listenBtn && audioPlayBtn && audioProgressBar && audioTimeDisplay && audioProgressContainer) {
          ...
          audioTest.src = audioSrc;
          
          // Listen for both error and loadedmetadata events
          audioTest.addEventListener('error', () => {
            console.warn('Audio file could not be loaded:', audioSrc);
            // Keep the listen button hidden if the file doesn't exist
            listenBtn.style.display = 'none';
          });
          
          audioTest.addEventListener('loadedmetadata', () => {
            // Audio file exists and has metadata, show the button
